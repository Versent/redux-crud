...
Takes on record or an array of records and adds them to the current state. Uses the given `key` or `id` by default to merge.
...
Also add these two properties to the added records:
...
This reducer will remove `busy` from the given records. It will not rollback the records to their previous state as we don't want users to lose their changes. The records will keep the `unsaved` attribute set to true.
...
Marks the given records as `deleted` and `busy`. This reducer doesn't actually remove them.
In your UI filter our records with `deleted`.
...
Removes `deleted` and `busy` from the given records.
